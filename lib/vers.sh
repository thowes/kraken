...
case $1 in
	...
	-w*)
		case $2 in
			...
			*) if [ -f $DIR_CFG/versions.txt ]; then rm $DIR_CFG/versions.txt; fi; echo "versions.txt:" $(pvm) > $DIR_CFG/versions.txt; vers -w git; vers -w gpg; vers -w heroku; vers_w mysql; vers_w php; vers_w psql; vers_w python; vers -w rsync; vers -w wget;;
		esac;; 
	*)

...
if [ $TESTATTR == "__" ]; then
	KR_TO_DIR=$(pwd)/cfg
	if [ -d 'cfg' ]; then
		if [ -f cfg/alias.sh ]; then echo "[[ ERR INSTALL alias.sh exists already in the directory! ]]"; else cp $KR_FROM_DIR/def/$KR_INSTALL_ALIAS_FILE cfg/alias.sh; fi
		...
		if [ -f $KR_FROM_DIR/def/$KR_INSTALL_ALIAS_FILE ]; then cp $KR_FROM_DIR/def/$KR_INSTALL_ALIAS_FILE cfg/alias.sh; fi
